...
const DestinationFactory = {
  create: ({ queueName, taskType, modification, doesCompletePreviousTask, group, onTrue, onFalse, eventNames, destinations, doesCompleteItem }) => {
    if (queueName) {
      return new UnconditionalDestination({ queueName, taskType, modification, doesCompletePreviousTask });
    }
    if (group) {
